...
function initForm() {
    ...
    if (form) {
        ...
        const now = new Date();
        if (selectedDate <= now) {
            tg.showAlert('Пожалуйста, выберите будущую дату и время');
            return;
        }
        ...
        try {
            // Отправляем данные напрямую в Bot API
            console.log('📤 Отправка данных в Bot API...', formData);
            ...
            // Отправляем в Bot API
            const botApiUrl = BOT_API_CONFIG.apiUrl;
            const result = await sendToBotAPI({
                ...formData,
                user_id: userId,
                username: username
            }, botApiUrl);
            ...
            if (result.success) {
                console.log('✅ Запись отправлена успешно:', result);
                const successMsg = `Запись #${result.appointment_id} отправлена! Мы свяжемся с вами.`;
                showSuccessMessage(successMsg);
                form.reset();
                if (document.getElementById('commentCount')) {
                    document.getElementById('commentCount').textContent = '0';
                }
                tg.showAlert('✅ Запись успешно отправлена!');
            } else {
                console.error('❌ Ошибка отправки:', result.message);
                showErrorMessage(result.message || 'Произошла ошибка при отправке записи');
                tg.showAlert('❌ Ошибка: ' + (result.message || 'Попробуйте позже'));
            }
...
// ============================================
// ФУНКЦИИ ДЛЯ ПРЯМОЙ ОТПРАВКИ В TELEGRAM И БД
// ============================================

async function sendToBotAPI(formData, apiUrl) {
    console.log('📤 Отправка данных в Bot API...', formData);
    ...
    try {
        ...
        console.log('✅ Ответ от Bot API:', result);
        
        return result;
        
    } catch (error) {
        console.error('❌ Ошибка отправки в Bot API:', error);
        return {
            success: false,
...
async function createGitHubIssue(formData) {
    console.log('📤 Создание GitHub Issue...', formData);
    ...
    const title = `Запись: ${formData.name} на ${formattedDate}`;
    
    // Формируем тело Issue в Markdown
    let body = `## 📋 Детали записи\n\n`;
    body += `**👤 Имя:** ${escapeMarkdown(formData.name)}\n`;
    body += `**📞 Телефон:** ${escapeMarkdown(formData.phone)}\n`;
    body += `**💅 Услуга:** ${escapeMarkdown(formData.service)}\n`;
    body += `**📅 Дата и время:** ${formattedDate}\n`;
    ...
    if (userId) {
        ...
        body += `- ID: ${userId}\n`;
        if (userUsername) {
            body += `- Username: @${escapeMarkdown(userUsername)}\n`;
        }
    }
    
    if (formData.comment) {
        body += `\n**💬 Комментарий:**\n${escapeMarkdown(formData.comment)}\n`;
    }
...
function getGitHubToken() {
    // Вариант 1: Из конфига (только для разработки!)
    if (GITHUB_CONFIG.token) {
        return GITHUB_CONFIG.token;
    }
    
    // Вариант 2: Из localStorage (небезопасно, только для тестирования)
