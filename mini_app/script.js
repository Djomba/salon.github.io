...
function initForm() {
    ...
    if (form) {
        ...
        try {
            // Отправляем данные в GitHub Gist
            console.log('📤 Отправка данных в GitHub Gist...', formData);
            ...
            // Проверяем, что GitHub конфигурация настроена
            if (typeof GITHUB_CONFIG === 'undefined') {
                throw new Error('GITHUB_CONFIG не найден. Убедитесь, что config.js загружен перед script.js.');
            }
            
            if (!GITHUB_CONFIG || !GITHUB_CONFIG.token) {
                throw new Error('⚠️ GitHub токен не настроен! Откройте config.js и вставьте ваш GitHub Personal Access Token в поле GITHUB_CONFIG.token');
            }
            
            // Проверяем, что функция saveToGitHubGist доступна
            if (typeof saveToGitHubGist === 'undefined') {
                throw new Error('saveToGitHubGist не найден. Убедитесь, что github-handler.js загружен.');
            }
            
            // Сохраняем в GitHub Gist
            const result = await saveToGitHubGist(dataToSend);
            
            if (result.success) {
                console.log('✅ Данные успешно сохранены в GitHub Gist:', result);
                ...
                // Сохраняем Gist ID если это первый раз
                if (result.gistId && !GITHUB_CONFIG.gistId) {
                    console.log('💡 Сохраните этот Gist ID в config.js:', result.gistId);
                    console.log('💡 GITHUB_CONFIG.gistId = "' + result.gistId + '"');
                }
            } else {
